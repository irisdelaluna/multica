...
// shape; unstamped builds report `dev`, and tagged releases are bare semver.
// Treating source-built daemons as OK keeps local development unblocked without
// weakening the gate for staging or production users running stale releases.
 ...
 * Dev-built daemons (`dev` or git-describe shape) are always OK — the version
 * string itself is the shared signal, so frontend and server agree.
 */
 ...
 * all degrade to `false`, and dev-built daemons (`dev` or git-describe shape)
 * always pass — the version string is the shared signal, so frontend and
 * server agree. Pure and synchronous, so the note box can settle from the
...
export function handoffSupported(detected: string | undefined | null): boolean {
  ...
  if (current === "dev" || DEV_DESCRIBE_RE.test(current)) return true;
